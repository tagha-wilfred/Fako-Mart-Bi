...
function onOpen() {
  SpreadsheetApp.getUi()
    ...
    .addItem("Open Reports Panel", "openFakoSidebar")
    .addSeparator()
    .addItem("Preview Employee Report (Config Sheet)", "previewEmployeeReport")
    .addItem("Preview Client Report (Config Sheet)",   "previewClientReport")
    .addToUi();
}

function openFakoSidebar() {
  var tmpl     = HtmlService.createTemplateFromFile("FakoSidebar");
  tmpl.logoUrl = fmLogo;
  var html = tmpl.evaluate()
    .setTitle("Fako Mart Reports Panel")
    .setWidth(310);
